...
def cargar_modulo():
    ...
    spec = importlib.util.spec_from_file_location("validar_v2_observabilidad_costes", ruta)
...
def test_escribir_validacion_crea_markdown(tmp_path):
    ...
    resultado = modulo.construir_resultado(tmp_path)
    salida = tmp_path / "salidas" / "validacion_v2_observabilidad_costes.md"
    ruta = modulo.escribir_validacion(tmp_path, resultado, salida)

    assert ruta.exists()
    texto = ruta.read_text(encoding="utf-8")
    assert "VALIDACIÓN V2" in texto
    assert "VALIDACION_V2_OBSERVABILIDAD_COSTES: OK" in texto
