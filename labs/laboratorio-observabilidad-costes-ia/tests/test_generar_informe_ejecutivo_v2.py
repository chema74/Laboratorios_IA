...
def cargar_modulo():
    ...
    spec = importlib.util.spec_from_file_location("generar_informe_ejecutivo_v2_observabilidad_costes", ruta)
...
def crear_estructura_minima(base: Path) -> None:
    ...
    for nombre in [
        "PLAN_V2_LABORATORIO_OBSERVABILIDAD_COSTES.md",
    ...
    (salidas / "validacion_v2_observabilidad_costes.md").write_text(
...
def test_construir_informe_incluye_estado_y_licencia(tmp_path):
    ...
    assert "LABORATORIO OBSERVABILIDAD Y COSTES IA" in informe
...
def test_escribir_informe_crea_archivo_markdown(tmp_path):
    ...
    salida = tmp_path / "salidas" / "informe_ejecutivo_v2_observabilidad_costes.md"
    ruta = modulo.escribir_informe(tmp_path, salida)

    assert ruta.exists()
    texto = ruta.read_text(encoding="utf-8")
    assert "INFORME_EJECUTIVO_V2_OBSERVABILIDAD_COSTES_GENERADO: OK" in texto
