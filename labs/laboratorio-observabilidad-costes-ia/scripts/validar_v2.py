...
def listar_markdown(base: Path) -> list[str]:
    docs = base / "docs"
    if not docs.exists():
        return []
    return sorted(str(r.relative_to(base)).replace("\\", "/") for r in docs.glob("*.md"))
...
def comprobar_licencias_v2(base: Path) -> list[dict[str, object]]:
    ...
    for ruta_relativa in DOCUMENTOS_V2:
        ...
        resultados.append({"ruta": ruta_relativa, "licencia_ok": licencia_ok})
...
def construir_resultado(base: Path) -> dict[str, object]:
    ...
    documentos_markdown = listar_markdown(base)
    ...
    resultado_ok = (
        ...
        and len(documentos_markdown) >= len(DOCUMENTOS_V2)
...
def formatear_markdown(resultado: dict[str, object]) -> str:
    lineas = [
        "# 🧪 VALIDACIÓN V2 — LABORATORIO OBSERVABILIDAD Y COSTES IA",
        "",
        "## 1. Resultado",
        "",
    ...
    lineas.extend(["", "---", "", "## 3. Scripts Python detectados", ""])
    for ruta in resultado["scripts_python"]:
        lineas.append(f"- `OK` — `{ruta}`")

    lineas.extend(["", "---", "", "## 4. Tests Python detectados", ""])
    for ruta in resultado["tests_python"]:
        lineas.append(f"- `OK` — `{ruta}`")

    lineas.extend(["", "---", "", "## 5. Licencias V2", ""])
    for item in resultado["licencias_v2"]:
        estado = "OK" if item["licencia_ok"] else "FALTA"
        lineas.append(f"- `{estado}` — `{item['ruta']}`")

    lineas.extend(["", "---", "", "## 6. Revisión de mojibake", ""])
    ...
    lineas.extend([
        ...
        "## 7. Estado final",
        "",
        f"`VALIDACION_V2_OBSERVABILIDAD_COSTES: {str(resultado['resultado']).upper()}`",
        "",
        "`WEB_PUBLICA: NO_MODIFICADA`",
        "",
        "`MAIN: NO_MODIFICADO`",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Valida la estructura V2 del laboratorio de observabilidad y costes IA.")
    parser.add_argument("--salida", default="salidas/validacion_v2_observabilidad_costes.md")
    ...
    if args.json:
        ...
        print(f"OK_VALIDACION_V2_OBSERVABILIDAD_COSTES: {salida}")
