...
def formatear_markdown(resultado: dict[str, object]) -> str:
    lineas = [
        "# 🧪 VALIDACIÓN V2 — LABORATORIO GOBERNANZA AI ACT LOCAL",
        "",
        "## 1. Resultado",
        "",
    ...
    lineas.extend([
        ...
        "## 5. Estado final",
        "",
        f"`VALIDACION_V2_GOBERNANZA_AI_ACT: {str(resultado['resultado']).upper()}`",
        "",
        "`WEB_PUBLICA: NO_MODIFICADA`",
        "",
        "`MAIN: NO_MODIFICADO`",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Valida la estructura V2 del laboratorio de gobernanza AI Act local.")
    parser.add_argument("--salida", default="salidas/validacion_v2_gobernanza_ai_act.md")
    ...
    if args.json:
        ...
        print(f"OK_VALIDACION_V2_GOBERNANZA_AI_ACT: {salida}")
