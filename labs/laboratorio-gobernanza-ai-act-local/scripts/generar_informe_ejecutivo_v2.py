...
DOCUMENTOS_CLAVE = [
    "docs/PLAN_V2_LABORATORIO_GOBERNANZA_AI_ACT.md",
    "docs/MAPA_EVIDENCIAS_V2.md",
    "docs/LIMITES_ALCANCE_V2.md",
    "docs/GUIA_REVISION_EVIDENCIAS_V2.md",
]

SALIDAS_CLAVE = ["salidas/validacion_v2_gobernanza_ai_act.md"]
...
def detectar_resultado_validacion(texto: str) -> str:
    patrones = [
        ...
        r"VALIDACION_V2_GOBERNANZA_AI_ACT:\s*([A-Z]+)",
        r'"resultado"\s*:\s*"([^"]+)"',
    ]
...
def construir_informe(base: Path) -> str:
    ...
    texto_validacion = leer_texto(base / "salidas" / "validacion_v2_gobernanza_ai_act.md")
    resultado_validacion = detectar_resultado_validacion(texto_validacion)
    fecha = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")

    lineas = [
        "# 📌 INFORME EJECUTIVO V2 — LABORATORIO GOBERNANZA AI ACT LOCAL",
        "",
        "## 1. Objetivo",
        "",
        "Este informe resume el estado ejecutivo de la V2 (Version 2 – Versión 2) del laboratorio `laboratorio-gobernanza-ai-act-local`.",
        "",
        "El laboratorio demuestra una base local para gobernanza técnica de IA (Artificial Intelligence – Inteligencia Artificial) con referencia orientativa al AI Act (Artificial Intelligence Act – Reglamento Europeo de Inteligencia Artificial).",
        "",
        ...
        "",
        "Este informe no constituye asesoría legal.",
        "",
        "---",
        "",
    ...
    lineas.extend([
        ...
        "",
        "El laboratorio representa una base controlada para hablar de inventario, evidencias, límites y trazabilidad de gobernanza IA en contexto local.",
        "",
        "Su valor no está en certificar cumplimiento legal, sino en demostrar criterio técnico sobre cómo ordenar evidencias de gobernanza.",
        "",
        "---",
        "",
        "## 5. Límites reconocidos",
        "",
        "- No es asesoría legal.",
        "- No certifica cumplimiento del AI Act.",
        "- No sustituye auditoría jurídica.",
        ...
        "## 6. Estado final",
        "",
        "`INFORME_EJECUTIVO_V2_GOBERNANZA_AI_ACT_GENERADO: OK`",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Genera informe ejecutivo V2 de gobernanza AI Act local.")
    parser.add_argument("--salida", default="salidas/informe_ejecutivo_v2_gobernanza_ai_act.md")
    ...
    if args.json:
        ...
        print(f"OK_INFORME_EJECUTIVO_V2_GOBERNANZA_AI_ACT: {salida}")
