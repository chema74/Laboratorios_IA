...
SALIDAS_CLAVE = ["salidas/validacion_v2_agentes_enterprise.md"]
...
def detectar_resultado_validacion(texto: str) -> str:
    patrones = [
        ...
        r"VALIDACION_V2_AGENTES_ENTERPRISE:\s*([A-Z]+)",
        r'"resultado"\s*:\s*"([^"]+)"',
    ]
...
def construir_informe(base: Path) -> str:
    ...
    texto_validacion = leer_texto(base / "salidas" / "validacion_v2_agentes_enterprise.md")
    resultado_validacion = detectar_resultado_validacion(texto_validacion)
    fecha = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")

    lineas = [
        "# 📌 INFORME EJECUTIVO V2 — LABORATORIO AGENTES ENTERPRISE LOCAL",
        "",
        "## 1. Objetivo",
        "",
        "Este informe resume el estado ejecutivo de la V2 (Version 2 – Versión 2) del laboratorio `laboratorio-agentes-enterprise-local`.",
        "",
        "El laboratorio demuestra una base local para agentes de IA (Artificial Intelligence – Inteligencia Artificial) orientados a escenarios empresariales, con validación, trazabilidad y límites explícitos.",
        "",
    ...
    lineas.extend([
        ...
        "",
        "El laboratorio representa una base controlada para hablar de agentes enterprise locales: componentes, responsabilidades, validación, límites y trazabilidad.",
        "",
        "Su valor no está en prometer una plataforma definitiva, sino en demostrar criterio técnico sobre cómo estructurar y evaluar agentes de IA en un entorno empresarial local.",
        "",
        "---",
        "",
        "## 5. Límites reconocidos",
        "",
        "- No es una plataforma enterprise productiva completa.",
        "- No sustituye seguridad, permisos ni observabilidad productiva.",
        ...
        "## 6. Estado final",
        "",
        "`INFORME_EJECUTIVO_V2_AGENTES_ENTERPRISE_GENERADO: OK`",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Genera informe ejecutivo V2 de agentes enterprise local.")
    parser.add_argument("--salida", default="salidas/informe_ejecutivo_v2_agentes_enterprise.md")
    ...
    if args.json:
        ...
        print(f"OK_INFORME_EJECUTIVO_V2_AGENTES_ENTERPRISE: {salida}")
